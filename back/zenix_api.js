...
app.post('/agencia/analise', async (req, res) => {
  ...
  const prompt = `Sou um assistente bancário. Uma agência chamada ${nome} está ${status ? 'aberta' : 'fechada'}, funcionando das ${horario}, e ${manutencao ? 'com manutenção em andamento' : 'sem manutenção'}. Dê uma resposta amigável explicando a situação para o cliente.`
